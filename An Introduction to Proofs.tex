...
\end{problem}
\begin{problem}
    A \hdef{Baumslag-Solitar group} $BS(m,n)$ admits the presentation $\cyc{a,b\mid b{a}^{m}\inv{b}={a}^{n}}$. Prove that $\vp:BS(m,n)\to BS(m,n)$ defined by $\vp(a)={a}^{2}$ and $\vp(b)=b$ is a group epimorphism and the commutator $[ba\inv{b},a]$ represents an element in $\ker(\vp)$. This morphism $\vp$ is actually not an isomorphism, but this statement requires advanced techniques to prove.
\end{problem}
