...
\end{definition}
\begin{definition}
    An \hdef{initial object} $0$ of a category $\cft{C}$ is an object such that for any object $A$, there is a unique morphism $0\to A$. A \hdef{terminal object} $1$ of a category $\cft{C}$ is an object such that for any object $B$, there is a unique morphism $A\to 1$.
\end{definition}
\begin{proposition}
    Initial object and terminal object of a category are unique up to isomorphism.
\end{proposition}
\begin{proof}
...
\par
Consider a relation $\sim\ \sub\N\times\N$ such that $(a,b)\sim(c,d)$ if and only if $a+d=b+c$.
\begin{problem}
    Prove that $\sim$ is an equivalence relation.
\end{problem}
\begin{definition}
    The set of all \hdef{integers} is defined to be $\N\times{\N}_{\sim}$. 
\end{definition}
\begin{proposition}
