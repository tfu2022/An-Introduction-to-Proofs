...
\end{proof} 
\par
Intuitively, functions preserve the structure of sets. Now we require them to preserve more structure on sets.
\begin{definition}
    Let $(P,{\le}_{1})$ and $(Q,{\le}_{2})$ be partially ordered set and let $f:P\to Q$ be a function. We say $f$ is \hdef{order-preserving} if $x{\le}_{1}y$ implies $f(x){\le}_{2}f(y)$. If $f$ is also bijective, then we say $P$ and $Q$ have the same \hdef{order-type}.
\end{definition}
\begin{problem}
...
\end{problem}
\begin{problem}
    A \hdef{Burnside group} $B(m,n)$ of rank $m$ and exponent $n$ admits the presentation $B(m,n)=\cyc{{x}_{1},\dots,{x}_{m}\mid}$. Prove $B(m,2)$ is finite and give a formula of its order.
\end{problem}
\par
Every group can be written as a group presentation. We will show this later. 
