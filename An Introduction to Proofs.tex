...
\par
You may notice that we use different names for the same object—a common practice in mathematics. For example, the set of all real numbers $\R$ can be viewed as a set, a group, a ring, a field, a topological space, a metric space $\dots$ (hopefully you will learn every term here in this note) Each term highlights a particular aspect of the same structure. As Poincaré famously said, ``Mathematics is the art of giving the same name to different things.''
\par
...
\end{problem}
\begin{problem}
    Let $L$ be a partially-ordered set in which the maximum length of a subchain is finite. Then $L$ can be partitioned into $n$ antichains.
\end{problem}
\begin{definition}
...
\end{definition}
\begin{proposition}
    A function is injective if and only if it is a monomorphism in $\cft{Set}$.
\end{proposition}
\begin{proof}
...
\end{proof}
\begin{problem}
    Prove that a function is surjective if and only if it is an epimorphism in $\cft{Set}$.
\end{problem}
\begin{definition}
...
\end{problem}
\begin{definition}
    An \hdef{initial object} $0$ of a category $\cft{C}$ is an object in $\cft{C}$ such that for any object $A$, there is a unique morphism $0\to A$. A \hdef{terminal object} $1$ of a category $\cft{C}$ is an object of $\cft{C}$ such that for any object $B$, there is a unique morphism $A\to 1$. We say an object is a \hdef{zero object} if it is both an initial object and a terminal object.
\end{definition}
\begin{proposition}
...
\end{problem}
\begin{problem}
    Prove that $\es$ is the initial object in $\cft{Set}$. Prove that any singleton is a terminal object in $\cft{Set}$. Hence the zero object does not exist in $\cat{Set}$.
\end{problem}
...
\end{problem}
\par
Notice that the axiom schema of separation since it guarantees the existence of functions, so we need both axioms in ZFC system.
...
\begin{example}
    ...
    \begin{enumerate}
        \item The trivial group is the group of one element, that is, $\{e\}$.
...
\end{problem}
\begin{problem}
    Let $G$ be a group and let $g,h\in G$. Prove that the following statements hold.
    \begin{enumerate}
        \item $(-e)g=-g$;
        \item $-(-g)=g$;
        \item $-g(-h)=gh$.
...
\begin{definition}
    Let $G$ and $H$ be groups. A group $P$ is \hdef{projective} if $\ga:G\thra H$ and $\pi:P\to H$, then there exists a map $\vp:P\to G$ such that $\vp\ga=\pi$.
\end{definition}
\begin{center}
    \begin{tikzcd}
        G && H \\
        \\
        && P
        \arrow["\ga", two heads, from=1-1, to=1-3]
        \arrow["\vp", dashed, from=3-3, to=1-1]
        \arrow["\pi"', from=3-3, to=1-3]
    \end{tikzcd}
\end{center}
\begin{definition}
    Let $S\le G$ be a subgroup. A map $\rho:G\thra S$ is a \hdef{retraction} if ${\rho}^{2}=\rho$, and $S$ is called a \hdef{retract} of $G$.
\end{definition}
\begin{proposition}
    The projective groups are the retracts of free groups.
\end{proposition}



\begin{example}
    The \hdef{quaternion group} ${Q}_{8}$ is a group
\end{example}
...
\newpage
\section{Functors and Naturality}
