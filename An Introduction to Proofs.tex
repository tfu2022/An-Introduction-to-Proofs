...
\begin{definition}
    ...
    \begin{enumerate}
        \item a class, denoted $\Ob(\cft{C})$, of \hdef{objects};
        \item for each pair of objects $(X,Y)$, there exists a class ${\Hom}_{\cft{C}}(X,Y)$ of \hdef{morphisms} from $X$ to $Y$;
...
\begin{Universal property for quotient sets}
    Let $X$ and $Y$ be sets. Let $\sim$ be an equivalence relation on $X$ and let $f:X\to Y$ be invariant under the equivalence relation. Then there exists a function $\wb{f}:X/\sim Y$ such that $f=\wb{f}\comp\pi$.
...
by this universal property, that is, any set satisfying the universal property must be the quotient set.
\begin{proof}
