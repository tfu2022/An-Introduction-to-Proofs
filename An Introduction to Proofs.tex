...
% some card shit
\begin{definition}
    A infinite set $A$ is \hdef{countable} if there exists $f:\N\to A$ bijective. An infinite set that is not countable is called an uncountable set.
\end{definition}
...
\end{proof}
\begin{definition}
    Let $H\le G$ be a subgroup. We say $H$ is a \hdef{normal subgroup} of $G$, denoted $H\nsg G$, if $xH=Hx$ for all $x\in G$. A non-trivial group $G$ is said to be \hdef{simple} if the only subgroups of $G$ are $G$ and the trivial subgroup.
\end{definition}
\begin{problem}
