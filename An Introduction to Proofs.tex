...
\end{proposition}
\begin{proof}
    Denote the initial segement of ${W}_{i}$ by $x$ by ${I}_{i,x}$. Let $f:{W}_{1}\to{W}_{2}$ be a function defined by $f(x)=y$ if ${I}_{1,x}$ has the same order-type as ${I}_{2,y}$. 
...
\end{proposition}
\begin{proof}
    Let $P$ be well-ordered. For all $x\in P$, consider the initial segement ${I}_{x}$ given by $x$. If ${I}_{x}$ has the same order-type with some ordinal $\al$, then define $F(x)=\al$.
