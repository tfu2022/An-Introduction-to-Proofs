...
\par
The first section is a quick review of basic logic, the language we used in proofs. Sections 2 to 4 are set-theoretic constructions that are essential in any kind of math. Those sections discuss the axioms of sets in ZFC and their consequence. This part largely follows from [?] and [?].  In the later sections, those properties will be frequently used. Section 5 is an introduction to linear algebra. 

. The next two sections are introductory abstract algebra. We include discussions on groups, rings, and relations between rings and matrices. Section 8 is an algebraic construction of the real numbers $\R$. This section is entirely adopted from professor Lerman's Math 347 lectures given in 

In the last two sections, we offer a more general theory to think about every entity in mathematics, the theory of categories. Every 
...
\end{definition}
\par
It is trivial that any initial segement of a well-ordered set is well-ordered.
...
\begin{definition}
    Let $(P,{<}_{1})$ and $(Q,{<}_{2})$ be partially ordered set and let $f:P\to Q$ be a function. We say $f$ is \hdef{order-preserving} if $x{<}_{1}y$ implies $f(x){<}_{2}f(y)$. If $f$ is also bijective, then we say $P$ and $Q$ have the same \hdef{order-type}.
\end{definition}
\begin{problem}
...
\end{proof}
\begin{problem}
    Prove that any well-ordered set cannot have the same order-type as any of its initial segement.
\end{problem}
\begin{proposition}
    ...
    \begin{enumerate}
        ...
        \item ${W}_{1}$ has the same order-type as an initial segement of ${W}_{2}$.
        \item ${W}_{2}$ has the same order-type as an initial segement of ${W}_{1}$.
    \end{enumerate}
\end{proposition}
\begin{proof}
    Denote the initial segement of ${W}_{i}$ by $x$ by ${I}_{i,x}$. Let $f:{W}_{1}\to{W}_{2}$ be a function defined by $f(x)=y$ if ${I}_{1,x}$ has the same order-type as ${I}_{2,y}$. Suppose $f$ is not injective, then 

    Let $g$ be an order-preserving 
...
\end{proposition}
\begin{proof}
    Let $W$ be well-ordered. For all $x\in W$, consider the initial segement ${I}_{x}$ given by $x$. If ${I}_{x}$ has the same order-type with some ordinal $\al$, then define $F(x)=\al$. If $\al$ exists, then it must be unique, so $F$ is well-defined. By the axiom of replacement, $F(W)$ is a set. For all $x,y\in W$ and $x<y$, 
...
\end{problem}
\begin{definition}
    A triple $(G,\comp,e)$ is a \hdef{group} if it is a monoid and for all $g\in G$, there exists $h\in G$ such that $h\comp g=e=g\comp h$. Such $f$ is called an \hdef{inverse} of $g$. If $\comp$ is commutative, then the group is \hdef{abelian}. The \hdef{order} of a group $G$. denoted $\card{G}$, is the cardinality of the set $G$.
\end{definition}
\begin{example}
...
\end{definition}
\begin{problem}
    Take $(aN)(bN)=(ab)N$ as a composition on $G/N$, prove that $G/N$ is indeed a group.
\end{problem}
\begin{definition}
...
\end{center}
\par
We will first show the existence of free groups. Intuitively, there are no relations, except for the group axioms, among elements of the free group. 
\par
Consider an arbitrary set $X$. The elements of $X$ are called \hdef{generators}. Let $Y$ be a disjoint copy of $X$ with a bijection $f:X\to Y$. For all $x\in X$, if $f(x)=y$, then we say $y$ is the \hdef{inverse} of $x$, denoted $\inv{x}$. We can rewrite $Y=\inv{X}$ and denote ${X}^{\pm 1}=X\cup\inv{X}$.
\begin{definition}
    Let $X$ be a set. The elements of ${X}^{\pm 1}$ are called \hdef{letters}. A \hdef{word} $w$ is a finite sequence $({l}_{1},\dots,{l}_{n})$ of letters. The \hdef{length} of $w=({l}_{1},\dots,{l}_{n})$, denoted $\card{w}$, is the value $n$. If $n=0$, we call it the \hdef{empty word}, denoted 1. A word is \hdef{reduced} if it contains no part in the form $a\inv{a}$.
\end{definition}
\par
Let the set $F(X)$ be the set of all reduced words. 
...
\end{problem}
\begin{proposition}
    Let $X$ be a set, then $\cyc{X\mid\es}$ is the smallest group containing $X$.
\end{proposition}
\begin{proof}
...
\end{proof}
\begin{definition}
    Let $(X,{\T}_{X})$ be a topological space, let $S$ be a set, and let $\pi:X\thra S$ be surjective. The \hdef{quotient topology} on $S$ is the finest topology for which $\pi$ is continuous, and $\pi$ is called a \hdef{quotient map}.
\end{definition}
\begin{example}
    A \hdef{torus} is 
...
\section{Categorical Reformulations}
\par
