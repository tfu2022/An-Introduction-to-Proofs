...
\end{proof}
\begin{proposition} 
    The set $\Z$ and $\Q$ are countable.
\end{proposition}
\begin{proof}
    Since $\N$ is countable, $\N\times\N$ is countable. The natural projection $\N\times\N\to\Z$ is surjective. 
\end{proof}

%%%% it's better to introduce Cantor's theorem
%\customtheorem{Cantor's Theorem}
%\begin{Cantor's Theorem}
%    Let $S$ be arbitrary set (finite or infinite), the the sets $S$ and $2^S$ cannot have equal cardinality.
%\end{Cantor's Theorem}
%\begin{proof}
