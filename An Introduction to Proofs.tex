...
Know some new groups are important!
\begin{problem}
...
\end{problem}
\begin{problem}
    A \hdef{Burnside group} $B(m,n)$ is a group generated by $m$ elements such that for all $g\in B(m,n)$, ${g}^{n}=e$. Prove that $B(m,n)$ is finite if $B(m,n)$ is abelian. Use this property to show that $B(m,3)$ is finite.
\end{problem}
\begin{problem}
    A \hdef{Baumslag-Solitar group} $BS(m,n)$ admits the presentation $\cyc{a,b\mid b{a}^{m}\inv{b}={a}^{n}}$. Prove that $BS(1,1)\iso{C}_{2}$. Prove that $\vp:BS(m,n)\to BS(m,n)$ defined by $\vp(a)={a}^{2}$ and $\vp(b)=b$ is a group epimorphism.
\end{problem}
\begin{problem}
...
\section{Functors and Yoneda Lemma}
