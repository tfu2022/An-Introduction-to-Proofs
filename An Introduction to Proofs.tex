...
\begin{example}
    ...
    \begin{enumerate}
        ...
        \item Let $S$ be a set. All bijections $S\to S$ forms a group called the \hdef{symmetric group}. If $S$ is finite, then the symmetric group is denoted by ${S}_{n}$. An element of the symmetric group is called a \hdef{permutation}.
...
\end{example}
\par
Let $\pi$ be a permutation of ${S}_{n}$. A permuatation can be expressed by two-line notation, that is, 
\begin{center}
    $\pi=
    \begin{pmatrix}
        1 & 2 & \cdots & n \\
...
\par
A $k$-cycle is a cycle containing $k$ elements. The cycle type of a permutation is an expression of the form $({1}^{{m}_{1}},{2}^{{m}_{2}},\ldots,{n}^{{m}_{n}})$, where ${m}_{k}$ is the number of cycles of length $k$ in $\pi$. A 1-cycle of $\pi$ is called a fixedpoint. An \hdef{involution} is a permutation $\pi$ such that ${\pi}^{2}=\ve$, where $\ve$ is the identity.
\begin{problem}
    Prove that every permutation in ${S}_{n}$ can be written as a finite composition of involutions.
\end{problem}
\begin{definition}
    A permutation $\pi\in{S}_{n}$ is called an \hdef{odd permutation} if it can be written as the composition of an odd number of involutions. A permutation $\pi\in{S}_{n}$ is called an \hdef{even permutation} if it can be written as the composition of an even number of involutions.
\end{definition}
\begin{proposition}
...
\begin{theorem}[Zassenhaus lemma]
    Let $U$ and $V$ be subgroups of a group. Let $u$ and $v$ be normal subgroups of $U$ and $V$, respectively. Then $u(U\cap v)$ is normal in $u(U\cap V)$, $(u\cap V)v$ is normal in $(U\cap V)v$, and $u(U\cap V)/u(U\cap v)\iso(U\cap V)v/(u\cap V)v$.
\end{theorem}
\begin{center}
    \begin{tikzpicture}
