...
\begin{example}
    ...
    \begin{enumerate}
        ...
        \item Let $S$ be a set. All bijections $S\to S$ forms a group called the \hdef{symmetric group}. If $S$ is finite, then the symmetric group is denoted by ${S}_{n}$.
...
\end{problem}
\begin{proposition}
    Let $H\le G$ be a subgroup. Then there exists a list of left coset ${x}_{i}H$ such that ${x}_{n}H\cap{x}_{m}H=\es$ and $\Un{x}_{i}H=G$.
\end{proposition}
\begin{proof}
