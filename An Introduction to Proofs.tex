...
\end{theorem}
\begin{proof}
    Suppose $\al$ is an ordinal and $\al\notin C$, then $\al$ is not a successor ordinal and $\al$ is non-empty, so $\al$ is a limit ordinal. 
\end{proof}
...
\begin{remark}
    Well-ordering principle is based on ZF system, the set theory without axiom of choice. The well-ordering theorem is an equivalent form of axiom of choice, which is based on ZFC system. One can find a proof of the converse in [3].
\end{remark}
...
\end{theorem}
\begin{proof}
    By well-ordering theorem, $A$ and $B$ are well-ordered.
\end{proof}
...
\end{theorem}
\begin{remark}
    We already gave a proof of this theorem as a consequence of axiom of choice. However, axiom of choice is not necessary to prove this theorem, and we will give an alternative prove of it in ZF. 
\end{remark}
\par
This proof is technical, you may skip this part.
\begin{proof}
...
\end{definition}
\begin{hdefinition}[ring]
    A ring is an abelian group $(R,+,0)$ with operation $\cdot$ such that
    \begin{enumerate}
        \item $\cdot$ is associative: for all $ a, b, c\in R$, $a\cdot (b\cdot c)=(a\cdot b)\cdot c$.
        \item $\cdot$ distributes over $+$: for all $ a, b, c\in R, a\cdot (b+c)=a\cdot b+a\cdot c$ and $(a+b)\dot c=a\cdot c+b\cdot c$.
        \item There exists $ 1_{R}\in R$ such that $1_{R}\cdot a=a=a\cdot 1_{R}$ for all $ a\in R$. We call ${1}_{R}$ the \hdef{multiplicative identity} of $R$.
    \end{enumerate}
\end{hdefinition}
