...
\end{problem}
\section{Functions}
Functions are relations on sets that 
\begin{definition}
...
\end{proof}
\begin{definition}
    Let $\cft{C}$ be a category. Let $A$ and $B$ be objects in $\cft{C}$. The \hdef{product} of $A$ and $B$, denoted $C$, is an object in $\cft{C}$ such that the following diagram commutes.
\end{definition}
