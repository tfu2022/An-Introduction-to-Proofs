...
\end{proof}
\begin{definition}
    Let $(X,\le)$ be a well-ordered set. Let $x\in X$. Then the set $\{a\mid a\le x\}$ is called an \hdef{initial segment} of $X$ given by $x$.
\end{definition}
\par
...
\end{proposition}
\begin{problem}
    The \hdef{Heisenberg group} $H$ admits the presentation $\cyc{a,b,c\mid ca\inv{c}\inv{a},cb\inv{c}\inv{b},ba\inv{b}\inv{a}\inv{c}}$. Prove ${a}^{m}{b}^{n}{c}^{p}a={a}^{m}({b}^{n}a){c}^{p}={a}^{m+1}{b}^{n}{c}^{p+n}$, ${a}^{m}{b}^{n}{c}^{p}b={a}^{m}{b}^{n+1}{c}^{p}$, and ${a}^{m}{b}^{n}{c}^{p}c={a}^{m}{b}^{n}{c}^{p+1}$. Can we rewrite any element in $H$ by ${a}^{m}{b}^{n}{c}^{p}$? Prove or disprove it.
\end{problem}
\begin{problem}
