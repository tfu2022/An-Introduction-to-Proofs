...
\par
Many people have contributed to this short note. Special thanks to FSG, who wrote $\S 6$ and collaborated with me on a general topology note, an important reference for my writing in $\S 7$. Thanks to Tingyu Wu for collaborating $\S 4$. Thanks to Bryce for his suggestions on $\S 1$ and $\S 3$. 
\par
...
\end{proof}
\begin{proposition}
    Let $S$ be any set satisfying the universal property for quotient sets. Let $X$ be a set and let $\sim$ be an equivalence relation on $X$. Then $S\iso {X}_{\sim}$.
\end{proposition}
\begin{proof}
...
\end{center}
\begin{proof}
    Let $i:\im(f)\to B$ be the identity map, then $i$ is injective. The function $f:A\to B$ induces an equivalence relation $\sim$ on $X$. The projection map $\pi:A\to{A}_{\sim}$ is surjective. 
\end{proof}
