...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \item for each pair of objects $(X,Y)$, there exists a set ${\Hom}_{\cft{C}}(X,Y)$ of \hdef{morphisms} from $X$ to $Y$;
...
\end{example}
\begin{example}
    In the following diagram, each vertex is an object and arrow is a morphism. This defines a category.
    \begin{center}
        \begin{tikzcd}
            \bullet && \bullet
...
\end{example}
\begin{definition}
    Let $\cft{C}$ be a category. A morphism $f\in{\Hom}_{\cft{C}}(A,B)$ is an \hdef{isomorphism} if there exists $g\in{\Hom}_{\cft{C}}(B,A)$ such that $f\comp g={\id}_{B}$ and $g\comp f={\id}_{A}$.
\end{definition}
\begin{problem}
