...
\begin{Axiom of choice}
    Given an epimorphism in $\cat{Set}$, there exists a 
\end{Axiom of choice}
\section{Groups}
...
\end{problem}
\par
Given a group $G$, its group presentation might not be unique. Consider ${S}_{3}$ and ${D}_{6}$. 
...
\end{problem}
\begin{problem}
    A \hdef{Burnside group} $B(m,n)$ of rank $m$ and exponent $n$ is admits the presentation $B(m,n)=\cyc{{x}_{1},\dots,{x}_{m}\mid}$
\end{problem}
\begin{definition}
