...
\par
Many people have contributed to this short note. Special thanks to FSG, who wrote $\S 5$ and collaborated with me on a general topology note, an important reference for my writing in $\S 6$. Special thanks to Tingyu Wu for collaborating $\S 4$. Thanks to Bryce for his suggestions on $\S 1$ and $\S 3$. 
\par
...
\end{problem}
\begin{definition}
    Let $f:X\to Y$ be a function. Let $Z\sub Y$, then the \hdef{restriction} of $f$ onto $Z$ is the map $f{\vert}_{Z}:X\to Z$ defined by $f{\vert}_{Z}(x)=f(x)$ for all $x\in X$.
\end{definition}
\begin{problem}
    Let $f:X\to Y$ be a function. Prove that the restriction $f{\vert}_{\im(f)}$ is a well-defined surjective function.
\end{problem}
\begin{definition}
...
\end{problem}
\begin{definition}
    Let $X$ and $Y$ be sets. Let $\sim$ be an equivalence relation on $X$. A function $f:X\to Y$ is \hdef{invariant} under the equivalence relation such that, $x\sim y$ implies $f(x)=f(y)$.
\end{definition}
\begin{problem}
    Let $X$ be a set and let $\sim$ be an equivalence relation on $X$. Prove that $\pi$ is invariant under $\sim$.
\end{problem}
...
\end{center}
\par
The proof of the universal property has two parts. We first verify that the quotient set has the universal property.
Then we verify that the quotient set is characterized by this universal property, that is, any set satisfying the universal
property must be the quotient set. This proof will be separated into multiple propositions.
\begin{proposition}
    There exists a map $\wb{f}:X/\sim\to Y$ such that $f=\wb{f}\comp\pi$.
\end{proposition}
\begin{proof}
    Define the relation $\wb{f}=\{([x],y)\mid \}$. 
\end{proof}
...
\end{problem}
\begin{proposition}
    Any function $f:X\to Y$ induces an equivalence relation on $X$. 
\end{proposition}
\begin{proof}
    
\end{proof}
\begin{proposition}
    Let $S$ be any set satisfying the universal property for quotient sets. Let $X$ be a set and let $\sim$ be an equivalence relation on $X$. Then $S\iso X/\sim$.
\end{proposition}
\begin{proof}
    
\end{proof}
\par
This completes our proof of the universal property for quotients.
...
\begin{center}
    \begin{tikzcd}
        ...
        \arrow["{f{\vert}_{\im(f)}}"', curve={height=18pt}, from=1-1, to=1-5]
...
\end{center}
\begin{proof}
    Let $i:\im(f)\to B$ be the identity map, then $i$ is injective. The function $f:A\to B$ induces an equivalence relation $\sim$ on $X$. The projection map $\pi:A\to A/\sim$ is surjective. 
\end{proof}
