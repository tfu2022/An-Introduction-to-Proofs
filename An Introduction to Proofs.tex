...
\par
A $k$-cycle is a cycle containing $k$ elements. The cycle type of a permutation is an expression of the form $({1}^{{m}_{1}},{2}^{{m}_{2}},\ldots,{n}^{{m}_{n}})$, where ${m}_{k}$ is the number of cycles of length $k$ in $\pi$. An \hdef{involution} is a permutation $\pi$ such that ${\pi}^{2}=e$.
\begin{problem}
