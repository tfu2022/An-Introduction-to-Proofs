...
\begin{definition}
    Let $(X,\le)$ be a partially ordered set and let $Y$ be a nonempty subset of $X$. An element $a\in X$ is said to be an \hdef{upper bound} of $Y$ if for all $y\in Y$, $y\le a$. An element $a\in X$ is said to be a \hdef{lower bound} of $Y$ if for all $y\in Y$, $a\le y$. We call the least upper bound of $Y$ the \hdef{supremum} and we call the greatest lower bound of $Y$ the \hdef{infimum}.
\end{definition}
\begin{problem}
...
\end{Axiom of choice}
\par
We call such a function $f$ a \hdef{choice function}. Therefore, we can reformulate the axiom of choice as: for every nonempty set $X$, there exists a choice function.
\begin{remark}
    Throughout the notes, we shall see many equivalent forms of 
\end{remark}
...
\end{problem}
\begin{definition}
    Let $G$ be a group and let $S\sub G$. The set \hdef{generated} by $S$ is defined to be $\{\prod{s}_{i}^{\pm 1}\mid{s}_{i}\in S\}$. 
\end{definition}
...
\end{problem}
\begin{definition}
    Let $G$ be a group. A \hdef{commutator} of $G$ is an element of the form $xy\inv{x}\inv{y}$, where $x,y\in G$. Let ${G}^{c}$ be the subgroup of $G$ generated by all commutators in $G$. Here ${G}^{c}$ is called the \hdef{commutator subgroup} of $G$.
\end{definition}
\begin{problem}
    Prove ${G}^{c}$ is normal in $G$. Prove that $G/{G}^{c}$ is abelian, this is called the \hdef{abelianization} of $G$. Furthermore, prove that $G/{G}^{c}$ is the largest abelian subgroup of $G$.
\end{problem}
\begin{problem}
...
\end{problem}
\begin{problem}
    A \hdef{Baumslag-Solitar group} $BS(m,n)$ admits the presentation $\cyc{a,b\mid b{a}^{m}\inv{b}={a}^{n}}$. Prove that $BS(1,1)\iso{C}_{2}$. This shows that the group presentation of a group is not necessarily unique.
\end{problem}
\begin{problem}
...
\end{remark}
\begin{problem}
    Let $K$ be a field, let $\A{n}$ be the set $\{({a}_{1},\dots,{a}_{n})\mid{a}_{i}\in K\}$, and let $A=K[{x}_{1},\dots,{x}_{n}]$. The \hdef{zero locus} of $T\sub A$ is the set $Z(T)=\{P\in\A{n}\mid f\in T,f(P)=0\}$. A subset $Y$ of $\A{n}$ is an \hdef{algebraic set} if there exists a subset $T\sub A$ such that $Y=Z(T)$. Let the complements of the algebraic sets be the open sets, prove this defines a topology. This is called the \hdef{Zariski topology} on $\A{n}$.
\end{problem}
\begin{definition}
