...
\end{proposition}
\begin{proof}
    Since $\ca{R}=\ca{C}/I$ and $\ca{C}$ is a commutative ring, $\ca{R}$ is a commutative ring. Now we are left to show that multiplicative inverses exist for nonzero elements of $\ca{R}$. 
\end{proof}
\par
Let $a=[{a}_{k}],b=[{b}_{k}]\in\ca{R}$ be distinct. We define $a\le b$ if and only if ${a}_{k}\le{b}_{k}$ eventually.
\begin{definition}
    Let $F$ be a field and let $\le$ be a partial-ordering on $F$. We say $F$ is an \hdef{ordered field} if the following properties hold.
    \begin{enumerate}
        \item 
    \end{enumerate}
\end{definition}
