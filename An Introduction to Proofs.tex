...
\end{problem}
\begin{problem}
    Prove that the inclusion $\sub$ is not an equivalence relation in $\N$.
\end{problem}
\begin{problem}
...
\end{problem}
\begin{definition}
    Let $f:A\to B$ be a function. The function $g:B\to A$ is a \hdef{left inverse} of $f$ if $g\comp f={\id}_{A}$. The function $h:B\to A$ is a \hdef{right inverse} of $f$ if $f\comp h={\id}_{B}$. A function $\vp$ is called the \hdef{inverse} if it is both a left inverse and a right inverse of $f$.
\end{definition}
\begin{proposition}
...
\end{problem}
\begin{definition}
    A \hdef{category} consists of
    \begin{enumerate}
        \item a collection, denoted $\Ob$
    \end{enumerate}
\end{definition}
