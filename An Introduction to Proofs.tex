...
\end{definition}
\begin{remark}
    Although a class might not be a set, we extend the notion ``$\in$'' to classes. Let $C$ is a class, we write $x\in C$ if $x$ satisfies the formula $\vp$ corresponding to $C$.
\end{remark}
\begin{example}
...
\end{problem}
\begin{problem}
    Prove that $\vp:BS(2,3)\to BS(2,3)$ defined by $\vp(a)={a}^{2}$ and $\vp(b)=b$ is a group epimorphism.
\end{problem}
\begin{problem}
...
\section{Reformulation of Groups and Topologies}
