...
\end{problem}
\par
Consider a relation $\sim\ \sub\N\times\N$ such that $(a,b)\sim(c,d)$ if and only if $a+d=b+c$. We will denote the relation by $\sim$ in this section.
\begin{problem}
    Prove that $\sim$ is an equivalence relation.
\end{problem}
\begin{definition}
    The set of all \hdef{integers}, denoted $\Z$, is defined to be ${(\N\times{\N})}_{\sim}$. Let $[(a,b)]\in\Z$, the \hdef{inverse} of $[(a,b)]$ is defined to be $[(b,a)]$.
\end{definition}
\begin{problem}
